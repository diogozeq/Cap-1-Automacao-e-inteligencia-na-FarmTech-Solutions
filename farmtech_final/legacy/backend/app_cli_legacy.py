...
# --- Imports específicos ---
from sqlalchemy import create_engine, Column, Integer, Float, Boolean, DateTime, String, inspect, func as sqlfunc, event, text as _sa_text
...
try:
    from .initial_data import dados_coletados_fase3
except ImportError:
    logger.warning("Arquivo 'initial_data.py' não encontrado. Dados iniciais não serão populados automaticamente.")
    dados_coletados_fase3 = []

# --- Configurações ---
CONFIG_FILE = 'config.yaml'
DB_NAME_DEFAULT = 'data/farmtech.db'
...
# Resolve caminho absoluto do arquivo SQLite relativo a este diretório
_BACKEND_DIR = os.path.dirname(os.path.abspath(__file__))
_db_raw_path = config['db_name']  # pode ser relativo ou absoluto
if not os.path.isabs(_db_raw_path):
    _db_abs_path = os.path.join(_BACKEND_DIR, _db_raw_path)
else:
    _db_abs_path = _db_raw_path

# Garante que o diretório do banco exista
os.makedirs(os.path.dirname(_db_abs_path), exist_ok=True)

# Para Windows, substituir barras invertidas por barras normais na URL do SQLite
_db_url = _db_abs_path.replace("\\", "/")
engine = create_engine(f"sqlite:///{_db_url}", connect_args={"check_same_thread": False})
SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)

# --- MIGRAÇÃO AUTOMÁTICA PARA COLUNA 'emergencia' ---------------------------
from sqlalchemy import inspect as _sa_inspect, text as _sa_text  # noqa: E402
try:
    with engine.begin() as _conn:
        _insp = _sa_inspect(_conn)
        if _insp.has_table(TABLE_NAME):
            _cols = {c['name'] for c in _insp.get_columns(TABLE_NAME)}
            if 'emergencia' not in _cols:
                logger.warning("Coluna 'emergencia' ausente em '%s'. Adicionando...", TABLE_NAME)
                _conn.execute(_sa_text(f"ALTER TABLE {TABLE_NAME} ADD COLUMN emergencia INTEGER NOT NULL DEFAULT 0"))
                logger.info("Coluna 'emergencia' adicionada com sucesso.")
except Exception as _e:
    logger.error("Erro ao tentar migração automática da coluna 'emergencia': %s", _e)
...
class LeituraSensor(Base):
    ...
    def __repr__(self):
        return (f"<Leitura(id={self.id}, ts='{self.timestamp.strftime('%Y-%m-%d %H:%M')}', "
                ...
                f"T:{self.temperatura:.1f}°C, Bomba:{int(self.bomba_ligada)}, Emerg:{int(self.emergencia)})>")
        ...
        try:
            for item_dict in dados_originais_lista_de_dicts:
                ...
                leitura_obj = LeituraSensor(
                    ...
                    emergencia=bool(item_dict.get('emergencia', False))
                )
                db.add(leitura_obj)
            db.commit()
    ...
    try:
        ...
        with SessionLocal() as db:
            nova = LeituraSensor(timestamp=timestamp, umidade=umidade, ph_estimado=ph, fosforo_presente=fosforo,
                                 ...
                                 decisao_logica_esp32=dec_sug, emergencia=bool(bomba_final))
...
def treinar_ou_carregar_classificador_phd(df_historico, console_rich: RichConsole, cfg_ml: dict):
    ...
    if os.path.exists(MODEL_PATH):
        try:
            with open(MODEL_PATH, 'rb') as fp:
                clf_modelo_global = pickle.load(fp)
            console_rich.print('[green]Modelo RandomForest carregado de disco.[/]')
            return clf_modelo_global, getattr(clf_modelo_global, 'last_accuracy_', 0.0), None, None
        except Exception as e:
            logger.warning(f'Falha ao carregar modelo salvo: {e}. Re-treinando...')
    ...
    try:
        ...
        with open(MODEL_PATH, 'wb') as fp:
            pickle.dump(clf_modelo_global, fp)
        console_rich.print('[green]Modelo RandomForest salvo em disco.[/]')
    except Exception as e:
        logger.warning(f'Não foi possível salvar modelo: {e}')
...
def run_farmtech_phd_suite():
    ...
    else: console.print("[yellow]Aviso: 'initial_data.py' não encontrado ou vazio.[/]")
